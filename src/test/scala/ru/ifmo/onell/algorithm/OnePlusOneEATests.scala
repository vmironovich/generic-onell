...
class OnePlusOneEATests extends AnyFlatSpec with Matchers {
  "(1+1) EA" should "perform as expected on OneMax" in {
    ...
    val runs = IndexedSeq.fill(100)(new OnePlusOneEA(OnePlusOneEA.ResamplingMutation).optimize(om))
  ...
  it should "perform as expected on OneMaxPerm" in {
    ...
    val runs = IndexedSeq.fill(20)(new OnePlusOneEA(OnePlusOneEA.ResamplingMutation).optimize(om))
  ...
  it should "perform as expected on OneMaxPerm even if called through interface" in {
    val size = 200
    val om = new OneMaxPerm(size)
    val opo: Optimizer = new OnePlusOneEA(OnePlusOneEA.ResamplingMutation)
  ...
  it should "log improvements correctly" in {
    ...
    val calls = new OnePlusOneEA(OnePlusOneEA.ResamplingMutation).optimize(om, logger)
